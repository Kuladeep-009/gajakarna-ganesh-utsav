/**
 * QRCode for JavaScript (Standalone, zero external dependencies)
 * Based on Davidshimjs qrcodejs with cross-browser canvas/SVG rendering
 */
(function(){
...
(function(){
  // UTF-8 parser for QR8bitByte
  function parseUtf8(str){
    var back=[];for(var i=0;i<str.length;i++){
      var code=str.charCodeAt(i);
      if(code<=0x7f)back.push(code);
      else if(code<=0x7ff){back.push(0xc0|(code>>6));back.push(0x80|(code&0x3f))}
      else if(code<=0xffff){back.push(0xe0|(code>>12));back.push(0x80|((code>>6)&0x3f));back.push(0x80|(code&0x3f))}
      else{back.push(0xf0|(code>>18));back.push(0x80|((code>>12)&0x3f));back.push(0x80|((code>>6)&0x3f));back.push(0x80|(code&0x3f))}
    }
    return back;
  }
  QR8bitByte.prototype.parsedData = [];
  var origInit = QR8bitByte;
  QR8bitByte = function(data){
    this.mode = QRMode.MODE_8BIT_BYTE;
    this.data = data;
    this.parsedData = parseUtf8(data);
  };
  QR8bitByte.prototype = origInit.prototype;
})();
...
QRCodeModel.prototype={
  ...
  getBestMaskPattern:function(){
    ...
    for(var i=0;i<8;i++){
      this.makeImpl(true,i);var lostPoint=QRUtil.getLostPoint(this);
  ...
  setupTypeNumber:function(test){
    ...
    for(var i=0;i<18;i++){var mod=(!test&&((bits>>i)&1)==1);this.modules[Math.floor(i/3)][i%3+this.moduleCount-8-3]=mod;this.modules[i%3+this.moduleCount-8-3][Math.floor(i/3)]=mod}
  },
...
QRCodeModel.createData=function(typeNumber,errorCorrectLevel,dataList){
  ...
  for(var i=0;i<dataList.length;i++){
    var data=dataList[i];buffer.put(data.mode,4);
    buffer.put(data.getLength(),QRUtil.getLengthInBits(data.mode,typeNumber));
    data.write(buffer);
  }
...
QRRSBlock.getRSBlocks=function(typeNumber,errorCorrectLevel){
  ...
  var length=rsBlock.length/2;var list=[];
  for(var i=0;i<length;i++){
    var count=rsBlock[i*2+0];var totalCount=rsBlock[i*2+1];
    for(var j=0;j<count;j++)list.push(new QRRSBlock(totalCount,rsBlock[i*2+2]||(totalCount-rsBlock[1])));
  }
  // Standard fallback
  var res=[];
  for(var i=0;i<rsBlock.length;i+=3){
    var count=rsBlock[i];var totalCount=rsBlock[i+1];var dataCount=rsBlock[i+2];
    for(var j=0;j<count;j++)res.push(new QRRSBlock(totalCount,dataCount));
  }
 ...
 * Public QRCode API
 */
window.QRCode = function(targetEl, options){
  this._htOption = {
    width: 256,
    height: 256,
    typeNumber: 4,
